...
else 
{
    ...
    if($task == "Create Storm")
    {
       ...
       //if($stormID == null)$stormID=1;
        ...
        $createQry = oci_parse($c, "insert into WEMS_EVENT (EVENTID, EXTERNALID, EVENTTYPE, OPENTIME, EVENTYEAR, OPENUSER)
							VALUES (:EVENTID, :EXTERNALID, :EVENTTYPE, to_date(:OPENTIME, 'mm/dd/yyyy'), :EVENTYEAR, :OPENUSER)")
        							OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
        ...
        $noteQry = oci_parse($c, "insert into WEMS_EVENT_NOTES (EVENTID, EVENTTYPE, NOTETIME, NOTEUSER, EVENTUPDATE)
							VALUES (:EVENTID, :EVENTTYPE, to_date(:NOTETIME, 'mm/dd/yyyy HH24:MI:SS'), :NOTEUSER, :EVENTUPDATE)")
        							OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
        
        							oci_bind_by_name($noteQry, ":EVENTID",  $stormID, -1);
        							oci_bind_by_name($noteQry, ":EVENTTYPE", $eventType, -1);
        							oci_bind_by_name($noteQry, ":NOTETIME", $opentime, -1);
    ...
    if($task == "Assign Location")
    {
        ...
        //add any supporting docs
        
        
        $locationErrMsg = "";
       // $interlockingErrMsg = $task;
       ...
       // {
           ...
           // {
                ...
                for($i=0; $i<count($_FILES['fileToUpload']['name']); $i++)
                {
                    ...
                    else
                    {
                        ...
                        $sql = oci_parse($c, "INSERT INTO WEMS_LOCDOCS (EVENTID, MARKERID, BLOB_COL, ID) VALUES(:EVENTID, :MARKERID, empty_blob(), :ID) RETURNING BLOB_COL INTO :BLOB_COL");
        
                        oci_bind_by_name($sql, ":EVENTID", $eventID, -1);
                        oci_bind_by_name($sql, ":MARKERID", $lConponent, -1);
        ...
        // Update Location table 
                ...
                oci_bind_by_name($qry, ":ASSIGNLOC",  $lConponent, -1);
                //oci_bind_by_name($qry, ":FORMANID",   $lForman, -1);
                oci_bind_by_name($qry, ":EVENTID",   $eventID, -1);
        ...
        if($lConponent == 99999999) 
        {
            $qry = oci_parse($c, "update WEMS_CLEANABLE_TARGET SET NOTIFYTIME = to_date(:NOTIFYTIME, 'mm/dd/yyyy hh:mi AM'),
                                        ASSIGNED_SITEFOREMEN = :ASSIGNED_SITEFOREMEN, CT_STATUS = :CT_STATUS, CT_PASSNUM = :CTPASSNUM, CT_BAGS = :CTBAGS
                                        WHERE MARKERID = :MARKERID and TYPE = 'S'")
                                                    OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
                                                    ...
                                                    //SELECT THE EMP_ASSIGNED
                                                    
                                                    
                                                    $empAssign = "";
                                                    $openUser = "";
                                                    
                                                    $formanSQL = oci_parse($c, "select EMP_ASSIGNED, OPENUSER FROM WEMS_GANG WHERE FORMANID = :FORMANID")
                                                    OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
                                                    
                                                    oci_bind_by_name($formanSQL, ":FORMANID",  $lForman, -1);
                                                    
                                                    oci_execute($formanSQL);
                                                    
                                                    while($row = oci_fetch_array($formanSQL))
                                                    {
                                                    
                                                        $empAssign = $row['EMP_ASSIGNED'];
                                                        $openUser = $row['OPENUSER'];
                                                    
                                                    }
                                                     
                                                    
                                                    
                                                    //DELETE WHERE FORMANID  = ....
                                                    
                                                    $deleteFormanSQL = oci_parse($c, "delete FROM WEMS_GANG WHERE FORMANID = :FORMANID")
                                                    OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
                                                    
                                                    oci_bind_by_name($deleteFormanSQL, ":FORMANID",  $lForman, -1);
                                                    
                                                    oci_execute($deleteFormanSQL);
                                                    
                                                    
              $locQry = oci_parse($c, "select CTID from WEMS_CLEANABLE_TARGET WHERE MARKERID = :MARKERID")
                                                    OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
                                                    ...
                                                    while($row = oci_fetch_array($locQry))
                                                    {
                                                        ...
                                                        $ctQry = oci_parse($c, "insert into WEMS_GANG (EVENTID, FORMANID, EMP_ASSIGNED, OPENTIME, OPENUSER, ASSIGN_LOC)
                                                                                VALUES(:EVENTID, :FORMANID, :EMP_ASSIGNED, sysdate, :OPENUSER, :ASSIGN_LOC) ")
                                                                                OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
                                                                                                                                                                                    
                                                                                                  
                                                                                 oci_bind_by_name($ctQry, ":EVENTID",  $eventID, -1);
                                                                                 oci_bind_by_name($ctQry, ":FORMANID",  $lForman, -1);
                                                                                 oci_bind_by_name($ctQry, ":EMP_ASSIGNED", $empAssign, -1);
                                                                                 oci_bind_by_name($ctQry, ":OPENUSER",  $openUser, -1);
                                                                                 oci_bind_by_name($ctQry, ":ASSIGN_LOC", $ctid , -1);
                                                                   ...
                                                                   //Forman needs to be updated and added for another conponent??
                                                                   //the user picked an availible Gang and want the gang to be assigned to all conponents. 
                                                                   
                                                                                                  //Mark the Gang as assigned
                                                         // $qry = oci_parse($c, "update WEMS_GANG SET ASSIGN_LOC = :ASSIGNLOC WHERE FORMANID = :FORMANID and EVENTID = :EVENTID")
                                                         //                                         OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
                                                                                                   
                                                         //                                         oci_bind_by_name($qry, ":ASSIGNLOC",  $ctid, -1);
                                                         //                                         oci_bind_by_name($qry, ":FORMANID",   $lForman, -1);
                                                         //                                          oci_bind_by_name($qry, ":EVENTID",   $eventID, -1);
                                                                                                   
                                                         //                                         oci_execute($qry);
                                                                                                  
                                                          // $found = oci_num_rows($qry);
                                                          // echo "number of rows affected" . $found. ",            ";
                                                         
                                                          //if update did not update a record add record to WEMS_GANG                                        
                                                          //this will assign the GANG to multiple Conponents                                        
				    ...
				    ?>
							...
							?>
								...
								<tr>
									...
									<td>Start Date: </td><td><input readonly type="text" name="opentime" size="20" tabindex="24" id="opentime" value="<?php echo $openTime;?>"/><img src="cal.gif" width="16" border="0" id="startCalbutton" alt="Click here to pick date" /></td>
        				...
        				<br></br>
        									...
        									<table  align = "center" class="table" cellpadding="1" cellspacing="1" border="0" >
												...
												<tr>
													...
													if($eventID > 0)
													{
													   $qry = oci_parse($c, "select et.EVENTDESC, TO_CHAR(e.NOTETIME, 'MM/DD/YYYY') as NOTETIME, 
    	...
    	<div style="background-color:#FFF2F2;" id="view2"  > 
     			...
     			<form action="<?php echo $_SERVER['PHP_SELF']; ?>"  method="post" enctype="multipart/form-data" name="new_inquiry" id="mainform" >
					...
					<option value= "" >  </option>
                                   ...
                                   $qry = oci_parse($c, "SELECT EMPLOYEEID, NAME from EMPLOYEE order by NAME")
                                       OR die('Oracle error, in parse. Error: <pre>' . print_r(oci_error($c), 1) . '</pre>');
...
//------------------------------------------------------------------------------------------------------------------------------
    	...
    	function getEmployees() 
        {
          ...
          var loc = document.getElementById('lConponent').value;
