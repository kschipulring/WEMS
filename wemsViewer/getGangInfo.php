...
if ($formanID >= 0) {
	$qry = oci_parse($c, "SELECT EMP_ASSIGNED, ASSIGN_LOC, STATUS, OPENTIME
		from WEMS_GANG where FORMANID = : FORMANID and EVENTID = : EVENTID ")
 		...
 		oci_bind_by_name($qry, ":FORMANID", $formanID, -1); oci_bind_by_name($qry, ":EVENTID", $eventID, -1);
 		...
 		oci_bind_by_name($qry, ":FORMANID", $formanID, -1); oci_bind_by_name($qry, ":EVENTID", $eventID, -1); oci_execute($qry);
